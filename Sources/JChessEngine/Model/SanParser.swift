...
enum SanParser {
    ...
    static func parse(_ san: String) -> ParsedSAN {
        ...
        for c in body {
            if c.isLetter {
                disFile = Int(c.unicodeScalars.first!.value - Character("a").unicodeScalars.first!.value)
            } else if c.isNumber {
                disRank = Int(c.unicodeScalars.first!.value - Character("1").unicodeScalars.first!.value)
            }
        }
